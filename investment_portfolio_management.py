...
def customer_similarity_survey():
    ...
    return client_data_df, similarity_matrix

def plt_customer_similarity(client_data_df, similarity_matrix):
    # 计算每隔10个客户的刻度位置和标签
...
def data_preprocession():
    ...
    return mean_daily_returns,cov_matrix,returns_columns

# 定义一个函数来计算给定权重下投资组合的风险
def calculate_risk(weights, cov_matrix):
    portfolio_variance = np.dot(weights.T, np.dot(cov_matrix, weights))
    portfolio_risk = np.sqrt(portfolio_variance)
    # print('222222222',portfolio_risk)
    # is_scalar = np.isscalar(portfolio_risk)
    # print('111',weights)
    # 确保返回一个标量
...
# 定义一个函数，返回投资组合风险的平方
def minimize_risk_for_target_return(weights, possible_return, mean_daily_returns,cov_matrix):
...
def frontier(mean_daily_returns, returns_columns,cov_matrix):
    # 定义约束条件和边界条件
    ...
    # print('11111',frontier_y)
    for possible_return in frontier_y:
        result = minimize(minimize_risk_for_target_return, x0 = len(returns_columns) * [1. / len(returns_columns),], args = (possible_return,mean_daily_returns, cov_matrix), bounds=bounds, constraints=constraints)
    ...
    return bounds,constraints,efficient_frontier_df

def customer_risk_profile(efficient_frontier_df):
    # Load client data
    ...
    return risk_levels,target_risks,client_df,selected_portfolios_df

def plt_all(selected_portfolios_df,risk_levels,target_risks,returns_columns):
    # 客户风险偏好与选定投资组合的风险和收益
...
if __name__=='__main__':
    ...
    client_data_df, similarity_matrix = customer_similarity_survey()
    plt_customer_similarity(client_data_df, similarity_matrix)
    mean_daily_returns,cov_matrix,returns_columns = data_preprocession()
    # frontier(mean_daily_returns, returns_columns,cov_matrix)
    bounds,constraints,efficient_frontier_df = frontier(mean_daily_returns, returns_columns,cov_matrix)

    risk_levels,target_risks,client_df,selected_portfolios_df = customer_risk_profile(efficient_frontier_df)
    # plt_all(selected_portfolios_df,risk_levels,target_risks,returns_columns)
